...
var rule = {
    ...
    tab_order:['幽兰谣4K','bilibili'],//线路顺序,按里面的顺序优先，没写的依次排后面
    timeout:5000,
    ...
    if (/^http/.test(input)) {
        // 1. 定义解析接口地址
        let parserUrl = "https://xuexi.nxxz.asia/jx/qq_video_parser_v4.php?videoUrl=" + encodeURIComponent(input);
        
        // 2. 发起请求获取解析后的 JSON 数据
        // 注意：这里假设你的环境支持 request 函数
        let html = request(parserUrl);
        let result = {};
        
        try {
            let jRoot = JSON.parse(html);
            
            // 3. 假设接口返回的 JSON 中播放地址字段为 url 或 playUrl，请根据实际接口返回调整
            // 这里我用 jRoot["url"] 作为示例
            let playUrl = jRoot["url"] || jRoot["playUrl"] || jRoot["data"]; 
            
            if (playUrl) {
                result["parse"] = 0;
                result["url"] = playUrl;
                
                // 4. 伪装请求头（根据你处理视频爬虫的经验，这里建议保留 Referer 伪装）
                result["header"] = {
                    "Referer": "https://www.bilibili.com", // 或者根据视频来源动态修改
                ...
                print("解析失败：接口未返回播放地址");
                input = "";
            }
        } catch (e) {
            print("解析接口请求或 JSON 解析出错: " + e.message);
            input = "";
        }
    } else {
